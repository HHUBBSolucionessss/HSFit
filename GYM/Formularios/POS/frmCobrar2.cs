...
{
    public partial class frmCobrar2 : Form
    {
        frmPuntoVenta frm;
        int idVenta;
        ...
        public frmCobrar2(IWin32Window frm, int idVenta, decimal total, decimal cant, List<string> idProds, List<int> cants)
        {
            InitializeComponent();
        ...
        private void CerrarCuenta()
        {
            ...
            {                 
                ...
                sql.CommandText = "UPDATE venta SET fecha=NOW(), total_efectivo=?, estado=?, abierta=?, tipo_pago=?, folio_ticket=?, terminacion=? WHERE id=?";
            ...
            {
                Clases.FuncionesGenerales.MensajeError("Ha ocurrido un error al tratar de cerrar la cuenta.", ex);
            }
            catch (Exception ex)
            {
        ...
        private void AgregarMovimientoCaja()
        {
            ...
            {
                Clases.FuncionesGenerales.MensajeError("Ha ocurrido un error al tratar de registrar el movimiento en caja.", ex);
            }
            ...
            {
                Clases.FuncionesGenerales.MensajeError("Algún método invocado en AgregarMovimientoCaja no admite argumentos nulos.", ex);
            }
            catch (Exception ex)
            {
                Clases.FuncionesGenerales.MensajeError("Ha ocurrido un error genérico.", ex);
        ...
        private void DescontarInventario()
        {
            try
            {
                for (int i = 0; i < idProds.Count; i++)
                    Clases.Producto.DescontarInventario(idProds[i], cants[i]);
            }
            catch (MySqlException ex)
            {
                Clases.FuncionesGenerales.MensajeError("Ha ocurrido un error al tratar de descontar productos del inventario.", ex);
            }
            catch (Exception ex)
            {
                Clases.FuncionesGenerales.MensajeError("Ha ocurrido un error genérico.", ex);
                ...
                if (Clases.ConfiguracionXML.ExisteConfiguracion("ticket", "imprimir"))
                {
                    if (bool.Parse(Clases.ConfiguracionXML.LeerConfiguración("ticket", "imprimir")))
                    {
                        if (Clases.ConfiguracionXML.ExisteConfiguracion("ticket", "preguntar"))
                        {
                            ...
                            {
                                if (MessageBox.Show("¿Deseas imprimir el ticket de esta venta?", "HS FIT", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                                {
        ...
        private void CalcularCambio()
        {
            ...
            {
                Clases.FuncionesGenerales.MensajeError("Algún método invocado en CalcularCambio no admite argumentos nulos.", ex);
            }
        }
        ...
        private void chbTarjeta_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                Clases.FuncionesGenerales.MensajeError("Algún método llamado en el evento CheckedChanged del CheckBox Tarjeta no admite argumentos nulos.", ex);
            }
            catch (Exception ex)
            {
                Clases.FuncionesGenerales.MensajeError("Ha ocurrido un error genérico.", ex);
